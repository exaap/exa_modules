...
class ResPartner(models.Model):
    ...
    @api.multi
    @api.depends('customer', 'invoice_ids', 'invoice_ids.state',
                 'invoice_ids.amount_total', 'child_ids.invoice_ids',
    ...
    def _compute_risk_invoice(self):
        ...
        total_group = self.env['account.invoice'].sudo().read_group(
            [('type', 'in', ['out_invoice', 'out_refund']),
             ...
             ('partner_id', 'in', self.ids)], ['partner_id', 'amount_total'],
            ['partner_id'])
        for partner, child_ids in all_partners_and_children.items():
            partner.risk_invoice_draft = sum(
                x['amount_total'] for x in total_group
    ...
    def process_unpaid_invoices(self):
        ...
        groups = self.env['account.move.line'].sudo().read_group(
            [('reconciled', '=', False), ('partner_id', '!=', False),
             ('account_id.internal_type', '=', 'receivable'),
        ...
        ConfigParameter.set_param('partner_financial_risk.last_check',
                                  max_date)
        return True
