...
class ResPartner(models.Model):
    ...
    last_out_invoice_id = fields.Many2one(comodel_name='account.invoice',
    ...
    last_out_invoice_amount = fields.Monetary(
        ...
        copy=False,
        store=True)
    last_out_invoice_date = fields.Date(string="Last Customer Invoice Date",
                                        readonly=True,
                                        compute="_compute_last_invoice",
                                        copy=False,
                                        store=True)
    last_in_invoice_id = fields.Many2one(comodel_name='account.invoice',
    ...
    last_in_invoice_amount = fields.Monetary(
        ...
        copy=False,
        store=True)
    last_in_invoice_date = fields.Date(string="Last Supplier Invoice Date",
                                       readonly=True,
                                       compute="_compute_last_invoice",
                                       copy=False,
                                       store=True)
    ...
    def _compute_last_invoice(self):
        for partner in self:
            ...
            in_invoice_ids = self.env['account.invoice'].search([
                ('partner_id', '=', partner.id), ('type', '=', 'in_invoice'),
                ('state', 'not in', ['draft', 'cancel'])
            ])
