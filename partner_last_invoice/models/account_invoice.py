...
class AccountInvoice(models.Model):
    ...
    def _set_last_invoice(self):
        if self.type == 'out_invoice':
            ...
            if out_invoice:
                self.partner_id.write({
                    'last_out_invoice_id': out_invoice.id,
                    'last_out_invoice_amount': out_invoice.amount_total,
                    'last_out_invoice_date': out_invoice.date_invoice})
        ...
        if self.type == 'in_invoice':
            ...
            if in_invoice:
                self.partner_id.write({
                    'last_in_invoice_id': in_invoice.id,
                    'last_in_invoice_amount': in_invoice.amount_total,
                    'last_in_invoice_date': in_invoice.date_invoice})
