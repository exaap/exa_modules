...
class SaleOrder(models.Model):
    ...
    order_kit_lines = fields.One2many(comodel_name='sale.order.kit.line',
        ...
        for order_kit_line in self.order_kit_lines:
            for product_kit_line in order_kit_line.product_id.product_kit_lines:
                if order_kit_line.product_qty > 0 and product_kit_line.product_qty > 0:
                    ...
                    values.append((0, 0, {
                        'sequence':
                        order_kit_line.product_id.sequence,
                        'product_id':
                        product_kit_line.product_id.id,
                        'product_uom_qty':
                        order_kit_line.product_qty *
                        product_kit_line.product_qty,
                        'price_unit':
                        price_unit,
                        'is_product_kit':
                        True,
                    ...
                    raise UserError(
                        _('Product Qty Should be greater than Zero.'))
