...
class Covid19Survey(models.Model):
    ...
    partner_person_id = fields.Many2one('res.partner', string='Diligenciar con Persona Registrada',
        help="Si el usuario no existe por favor deje este campo vacío y llene los campos obligatorios.\nEl usuario quedará registrado cuando pulse en el botón guardar.", required=False)
    ...
    partner_country_id = fields.Many2one('res.country', string='País de Residencia', required=True, default=_default_country)
    ...
    def _compute_name(self):
        for partner in self:
            if partner.first_name and partner.last_name:
                complete_name = u' '.join([partner.first_name.upper(), partner.last_name.upper()])
    ...
    def createPartner(self, vals):
        ...
        if partner_id:
            return True
        raise UserError(_("No se ha podido crear el nuevo contacto.\nSi ya existe por favor búsquelo como 'Persona Registrada'."))
